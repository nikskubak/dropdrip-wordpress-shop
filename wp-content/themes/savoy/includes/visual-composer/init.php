	...
	if ( class_exists( 'WPBakeryVisualComposerAbstract' ) ) {
        ...
        function nm_vc_frontend_assets() {
            ...
            if ( ! $nm_globals['vcomp_stock'] ) {
                ...
                wp_enqueue_style( 'nm_js_composer_front', NM_THEME_URI . '/assets/css/visual-composer/nm-js_composer.css', array(), NM_THEME_VERSION, 'all' );

                // Enqueue frontend scripts (original file with unused scripts removed)
                wp_enqueue_script( 'nm_composer_front_js', NM_THEME_URI . '/assets/js/visual-composer/nm-js_composer_front.min.js', array( 'jquery' ), NM_THEME_VERSION, true );
            }
		...
		function nm_vc_register_shortcodes() {
			global $nm_theme_options, $nm_cf7_enabled;
			...
			if ( nm_woocommerce_activated() ) {
				...
				include( NM_VC_DIR . '/shortcodes/woocommerce/product-slider.php' );
			}
		}
		...
		if ( is_admin() ) {
			...
			function nm_vc_register_elements() {
				...
				if ( nm_woocommerce_activated() ) {
					...
					include( NM_VC_DIR . '/elements/woocommerce/product-slider.php' );
				}
			}
            ...
            if ( ! $nm_globals['vcomp_stock'] ) {
                ...
                function nm_vc_remove_template_library() {
                ...
                add_action( 'vc_after_init_base', 'nm_vc_remove_template_library' );
