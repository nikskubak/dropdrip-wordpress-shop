...
// NM: Custom version of the "WP_Importer" class - changes are commented with "NM"
...
if ( ! class_exists( 'WXR_Parser' ) ) {
    require NM_DIR . '/setup/includes/wordpress-importer/parsers.php';
}
// /NM
...
class NM_WP_Import extends WP_Importer {
// /NM
...
<?php
	...
	function process_attachment( $post, $url ) {
		...
		// NM: Use local folder for importing images
        /*if ( preg_match( '|^/[\w\W]+$|', $url ) )
			$url = rtrim( $this->base_url, '/' ) . $url;*/
        if ( defined( 'NM_IMPORT_LOCAL_IMAGES' ) && NM_IMPORT_LOCAL_IMAGES ) {
            $file_name = basename( $url );
            $url = NM_IMPORT_URI . '/setup/content/images/' . $file_name;
        } else {
            if ( preg_match( '|^/[\w\W]+$|', $url ) )
                $url = rtrim( $this->base_url, '/' ) . $url;
        }
